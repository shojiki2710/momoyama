...
Pipeline (see the Notion spec "仕様書：リスティンググループ商品ボード自動更新" for background):
  ...
  3. google_ads:      campaign, product_item_id, product_title, cost, conversions, conversions_value
                       -> per-product performance, summed by item_id (titles vary A/B).
  4. Join 2 and 3 on item_id, keep only items whose label maps to a currently-active AG,
     compute ROAS, render to static HTML.
...
class WindsorClient:
    ...
    def get_data(self, connector, fields, accounts, date_from, date_to):
        ...
        resp = requests.get(f"{WINDSOR_BASE_URL}/{connector}", params=params, timeout=60)
...
class FixtureClient:
    ...
    _FILES = {
        ...
        ("google_ads", ("campaign", "product_item_id", "product_title", "cost", "conversions", "conversions_value")): "step3_item_performance.json",
    }
...
def fetch_item_performance(client, date_from, date_to):
    rows = client.get_data(
        "google_ads",
        ["campaign", "product_item_id", "product_title", "cost", "conversions", "conversions_value"],
        accounts=[GOOGLE_ADS_ACCOUNT],
        date_from=date_from,
        date_to=date_to,
    )
    ...
    for row in rows:
        ...
        if not item_id or not any(k in campaign for k in CAMPAIGN_KEYWORDS):
            continue
        key = item_id.lower()
        bucket = agg.setdefault(key, {"cost": 0.0, "conversions": 0.0, "value": 0.0, "title": row.get("product_title")})
        bucket["cost"] += float(row.get("cost") or 0)
        bucket["conversions"] += float(row.get("conversions") or 0)
        bucket["value"] += float(row.get("conversions_value") or 0)
    return agg


def build_products(active_labels, product_labels, performance):
    products = []
    for item_id, perf in performance.items():
        info = product_labels.get(item_id)
        if not info or info["label"] not in active_labels:
            continue
        if perf["cost"] <= 0:
            continue
        roas = perf["value"] / perf["cost"] * 100
        products.append({
            "ag": LABEL_TO_DISPLAY_AG[info["label"]],
            "title": info["title"] or perf["title"] or item_id,
            "id": item_id,
            "img": info["image"],
            "cost": round(perf["cost"], 2),
            "cv": round(perf["conversions"], 4),
            "value": round(perf["value"], 2),
            "roas": round(roas, 1),
        })
    return products


def render_html(products, active_ag_names, date_from, date_to, generated_at):
    ...
    period_text = (
        f"{date_from}〜{date_to}／稼働中{len(ag_order)}AG"
        f"（{'・'.join(ag_order)}）・全{len(products)}商品"
    )
    ...
    html = html.replace("__PERIOD_TEXT__", js_string_escape(period_text))
...
def main():
    ...
    products = build_products(active_labels, product_labels, performance)
    ...
    html = render_html(products, active_ag_names, str(date_from), str(date_to), generated_at)

    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(html, encoding="utf-8")
    print(f"Wrote {args.out} ({len(products)} products, {len(active_ag_names)} active AGs).")
